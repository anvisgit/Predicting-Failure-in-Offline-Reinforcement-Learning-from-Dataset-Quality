...
Runs all 3 degradation protocols x 2 algorithms x 3 environments.
...
from src.data.degradation import get_coverage_variants, get_quality_variants, get_noise_variants
...
def parse_args():
    ...
    parser.add_argument('--env', type=str, default='all', choices=['halfcheetah', 'hopper', 'walker2d', 'all'])
    parser.add_argument('--protocol', type=str, default='all', choices=['coverage', 'quality', 'noise', 'all'])
...
def run_experiment(algo_name, env_name, protocol, level, degraded_dataset, env_obj, args):
    ...
    n_eval_episodes = 1 if args.quick else 10
    eval_res = evaluate_and_normalize(env_obj, env_name, agent.predict, n_episodes=n_eval_episodes)
...
def main():
    ...
    for env_name in envs:
        # Load base datasets needed for protocols
            ...
            for level, variant_data in variants.items():
                for algo in algos:
                    try:
                        ...
                        print(f"Error in experiment {algo} {env_name} {protocol} {level}: {e}")
