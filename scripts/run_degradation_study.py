...
def parse_args():
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Re-run experiments even if a results.json already exists (default: skip/resume)",
    )
...
def run_experiment(algo_name, env_name, protocol, level, degraded_dataset, env_obj, args):
    ...
    result_path = Path(args.results_dir) / protocol / run_name / "results.json"

    if result_path.exists() and not args.force:
        print(f"--- Skipping {run_name} (results.json already exists) ---")
        return
