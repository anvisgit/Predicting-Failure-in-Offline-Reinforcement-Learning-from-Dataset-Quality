...
ENV_CONFIGS = {
    ...
    "walker2d": {
        "obs_dim": 17,
        "act_dim": 6,
        "ref_min_score": 1.629008,
        "ref_max_score": 4592.3,
    },
}
...
def _load_hdf5(path: Path) -> Dict[str, np.ndarray]:
    """Load a D4RL HDF5 file into the common dataset dictionary format."""
...
def load_d4rl_dataset(env_name: str, dataset_variant: str) -> Tuple[Dict[str, np.ndarray], gym.Env]:
    """Load a D4RL dataset from Berkeley HDF5 files and a matching Gymnasium env."""
    ...
    env.dataset_name = get_d4rl_env_name(env_name, dataset_variant)
